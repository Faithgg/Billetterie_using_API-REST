...
<head>
    ...
    <title>Consommation de Billet</title>
    <style>
            @font-face {
            font-family: "plein2";
...
if ($methode === "GET") {
    ...
    $consume_key = filter_input(INPUT_GET, "code");
    if (!$nom || !$consume_key) {
        ?>
        <form action="consommation.php" method="GET">
        <fieldset class="center">
        <div class="band">
            <figcaption style="margin-left: 10%;font-size: 20px;font-family:plein2;">Formulaire de consommation de Billet</figcaption>
        </div>
            <div class="input">
            ...
            <div class="input">
                <label for="code">Code de consommation Billet: </label>
   ...
   <?php
        ...
        $pdo = connexion_BDD();
        $requette = $pdo->prepare("SELECT * FROM `billets` AS b INNER JOIN `visitors` AS v WHERE v.nom_prenom = :nom AND b.consume_code = :consume_key AND b.visitor_id = v.id ");
        $requette->execute([
            ":nom" => $nom,
...
?>
        ...
        <h2 style= "text-align:center;">    Billet inexistant    </h2>
<?php
        } else {
            if ($result[0]["consume"] === 1) {
            http_response_code(401);
            ?>
                <svg style="margin-left:24%;color: rgb(2, 240, 2);" xmlns="http://www.w3.org/2000/svg" width="50%" height="50%" fill="currentColor" class="bi bi-x" viewBox="0 0 16 16"> <path d="M4.646 4.646a.5.5 0 0 1 .708 0L8 7.293l2.646-2.647a.5.5 0 0 1 .708.708L8.707 8l2.647 2.646a.5.5 0 0 1-.708.708L8 8.707l-2.646 2.647a.5.5 0 0 1-.708-.708L7.293 8 4.646 5.354a.5.5 0 0 1 0-.708z" fill="#AE00C9"></path> </svg>
                <h2 style="text-align:center;">    Billet déjà consommé    </h2>
        <?php
            } else {
