...
if ($methode === "GET") {
    ...
    $_SESSION["update_event_id"] = filter_input(INPUT_GET, "id");
...
<?php
    ...
    <div class="band">
        <figcaption style="margin-left: 15%;font-size: 20px;font-family:plein2;">Formulaire de Modification d'Evenement</figcaption>
    </div>
    <div class="input">
        <label for="name_event">Nom de l'évenement:</label>
        <input type="text" id="name_event" name="name_event" placeholder="<?= $event_name?>" required>
    </div>
    <div class="input">
        <label for="lieu">Lieux où a lieu l'évenement:</label>
        <input type="text" id="lieu" name="lieu" placeholder ="<?= $event_place?>" required>
    </div>
    <div class="input">
        <label for="date">Date de l'évenement</label>
        <input type="date" id="date" name="date" value="<?= substr($event_datetime,0,10)?>" required>
    </div>
    <div class="input">
        <label for="heure">Heure de l'évenement</label>
        <input type="time" id="heure" name="heure" value="<?= substr($event_datetime,11,5)?>" required>
    </div>
    <div class="input_b">       
...
$_SESSION["donnees_justes"] = true;
    ...
    if (strlen ($name) === 0 || strlen ($lieu) === 0 || strlen ($date) === 0 || strlen ($hour) === 0 ){
        ...
        header("location:update.php?nom=$name&lieu=$lieu&date=$datetime&id=$id");
    } else {
            $pdo = connexion_BDD();
            $requete = $pdo->prepare("UPDATE events SET nom = :nom , Place = :lieu , `date` = :moment WHERE id= :id");
            $requete->execute( [
                ":nom" => $name,
                ":lieu" => $lieu,
                ":moment" => $datetime,
            ...
            $_SESSION["donnees_justes"] = true;
            unset($_SESSION["update_event_id"]); #J'en ai plus besoin        
